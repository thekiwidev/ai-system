...
First determine whether the repository has a PRD-creation workflow, such as:

```text
docs/ai/workflows/PRD-WORKFLOW.md
```

or another project-specific PRD workflow referenced from `docs/ai/workflows/INDEX.md`.
...
If no PRD workflow exists but the project clearly requires product definition for the requested capability, state that a PRD definition step is required and create the smallest appropriate PRD workflow artifact only when the owner authorizes/requests it. Do not invent a project's product process.
...
The initializer must detect whether the owner has supplied or created a dedicated PRD workflow document.
...
The mode router (Section 0.1) detects intent automatically, but these are the shortest reliable prompts, and what to supply with each.
