...
4. **The owner controls Git.** Inspect Git freely. Modify working-tree files when the task authorises implementation. **Never** create or switch branches, stage for convenience, commit, amend, rebase, merge, cherry-pick, reset, tag, release, push, or otherwise rewrite or publish history unless I explicitly authorise *that class of operation in the current interaction*. "Commit this" does not mean "push". When work is verified, say: *"Ready for your review and commit."* — and stop. Never phrase an unapproved Git operation as though it happened. **No AI attribution, ever (RULE-GIT-002):** commit messages, tags, PR text and release notes carry no `Co-Authored-By:` (or any other) trailer naming an AI, model or tool; no "Generated with …" / "Created by …" lines or robot emoji; no model or tool names; no AI identity as Git author or committer — this overrides any tool or harness default.
...
9. **Do what was asked — nothing more (RULE-SCOPE-002).** My instruction defines the deliverable. The workflow obligations of *that* change stay in scope (root cause, regression test, gates, changelog, memory, handoff, notes, context docs). Everything else — the same bug at other sites, audits and sweeps, refactors, team documents (release notes, QA checklists), any published or shared page, files carrying unrelated uncommitted edits — you **stop and ask** before doing. List what you found under "Found, not touched" in the report. Full rule: `rules/task-scope.md`.
...
- **Workflow first (RULE-KIND-002).** If the request matches a workflow's triggers — project `docs/ai/workflows/INDEX.md`, then global workflows — run that workflow exactly as written; invoking it approves every step in it. Not covered by the workflow → stop and ask, offer to update it.
- **Classify** otherwise: new feature · enhancement · bug fix · refactor · maintenance · docs-only · investigation. A new feature goes through product definition (the `create-prd` skill, then a plan, then my approval) before code. A bug fix establishes the root cause and a regression test before the fix.
...
- **Reuse before creation.** Search the codebase for the component, utility, schema or pattern that already does this. Smallest correct change — to the thing I named (§2.9).
- **Record**: after verifying, run a documentation impact review against the actual diff and reconcile every affected layer. Then report: classification · what changed · verification actually run and its result · documentation synchronised (per layer) · remaining work · decisions for me · Git status ("no Git operation performed; ready for review and commit"). With caveman on, the report is the labelled one-liners in `rules/caveman.md` § Reports. Changelog and notes entries follow `rules/docs-format.md` (versioned, unique sub-headings).
...
| `skills/<name>/SKILL.md` with `kind: workflow` | Repeatable operational runs (release, build, deploy) in one fixed shape | Run when the request matches its triggers. Create or update only with the `create-workflow` skill. |
...
**One shape per kind (RULE-KIND-001, `rules/kinds.md`).** Workflows, skills, rules, agents, plans, decisions and PRDs each have one home per scope, one template and one index. "Set up / update a …" goes through `kiwi-author` (workflows: `create-workflow`; PRDs: `create-prd`) — never a new ad-hoc format or location.
