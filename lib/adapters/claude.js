...
function global(ctx) {
  ...
  // The scope guard (RULE-SCOPE-001) and the caveman-mode hooks are always installed; the rest only with hooks.install.
  ops.push(hooksOp(ctx, home, { onlyGuard: !ctx.config.hooks.install }));
  return ops;
}

/** Hooks installed even when hooks.install is off: they enforce rules, not conveniences. */
const ALWAYS = /^(Scope guard|Caveman mode)/;

/** Claude Code settings that remove its commit and PR attribution (RULE-GIT-002). */
const NO_ATTRIBUTION = { includeCoAuthoredBy: false, attribution: { commit: '', pr: '' } };
...
function hooksOp(ctx, home, { onlyGuard = false } = {}) {
  ...
  return {
    ...
    agent: id,
    why: (onlyGuard ? 'scope guard + caveman-mode hooks' : 'kiwi hooks') + ' and no AI attribution (RULE-GIT-002) in settings.json (tagged [kiwi]; backup written)',
    merge(existing) {
      // RULE-GIT-002: no AI attribution in commits or PRs, whatever the harness default.
      const out = { ...existing, ...NO_ATTRIBUTION, attribution: { ...(existing.attribution || {}), ...NO_ATTRIBUTION.attribution }, hooks: { ...(existing.hooks || {}) } };
      for (const event of Object.keys(src)) {
        const kept = (out.hooks[event] || []).filter(e => !(e.description || '').startsWith(tag));
        const mine = src[event]
          .filter(e => !onlyGuard || ALWAYS.test(e.description || ''))
