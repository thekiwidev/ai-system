...
function validate(root) {
  ...
  for (const s of skills(root)) {
    ...
    if (s.data.kind === 'workflow') {
      const { workflowProblems } = require('./kinds');
      for (const problem of workflowProblems(fs.readFileSync(s.file, 'utf8'))) problems.push({ file: s.file, problem: `workflow: ${problem}` });
    }
  }
