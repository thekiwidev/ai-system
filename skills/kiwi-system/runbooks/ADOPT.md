...
8. **Tier 2 only where approved**: `domains/` (highest-risk first; §22.10 — never a directory listing), retro-ADRs (§22.8; 5–10 max; rationale `[inferred]` or "not recorded" — never fabricated), `plans/` for real planned work, project `workflows/` for real reusable procedures; `workflows/INDEX.md` references the global workflows by name.
