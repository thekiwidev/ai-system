...
5. **Write, in this order** (§21): `docs/ai/AGENT-CORE.md` → `INDEX.md` → `CONSTITUTION.md` → `WORKFLOW.md` → `ENGINEERING.md` → `VERIFICATION.md` (only commands that will genuinely exist; if none yet, say so in the file) → `ARCHITECTURE.md` (intended shape, clearly marked as intended until code exists) → `MEMORY.md` at the repo root → `HANDOFF.md` ("no active work") → `NOTES.md` → `CHANGELOG.md` (empty, in the chosen style) → `workflows/INDEX.md` (registry only — RULE-KIND-001; global `create-prd`, `feature-workflow`, `bugfix-workflow`, `create-workflow`, `kiwi-author`, `kiwi-system` by name; a workflow the owner already knows they repeat is created with `create-workflow`, never as a file in `docs/ai/workflows/`) → `plans/`, `decisions/`, `domains/` only if there is real content (§9.15–9.17; otherwise record as pruned) → `RULES.md` (register every rule you wrote) → `SYSTEM.md` **last** (§23; `mode_history` INIT; pruned modules with reasons; `open_confirmations`).
