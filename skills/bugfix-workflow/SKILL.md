...
Problem · evidence · root cause · smallest correct fix · regression test (fails before, passes after) · files expected to change · verification · documentation impact. For non-trivial fixes, wait for owner plan approval unless the project's active plan already authorises the work.

## 5. Fix

Write the regression test first; confirm it fails; apply the smallest correct fix; confirm it passes. No opportunistic refactoring inside a bug fix.

## 6. Verify

Run the project's verification gates (`docs/ai/VERIFICATION.md`) and the tests adjacent to the changed behaviour. Report actual results.

## 7. Record

- `CHANGELOG.md`: bug as experienced · who/what was affected · root cause · why existing checks allowed it · the actual fix · regression coverage · verification result. Never "fixed import issue" when the cause is known.
- `NOTES.md`: any remaining edge case, deliberate non-fix, or investigation context worth keeping.
...
Classification: bug fix · symptom → cause → fix · verification · documentation synchronized (per layer) · remaining work · *no Git operation performed; ready for your review and commit.*
